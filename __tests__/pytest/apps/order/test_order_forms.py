...
from apps.order.forms import (
  CreateOrderForm, UpdateOrderForm, OrderStatusForm, 
  OrderItemForm, AddOrderItemForm, UpdateOrderStatusForm
)

from __tests__.pytest.factories import OrderFactory, OrderLineItemFactory, ProductFactory, CategoryFactory
...
def test_create_order_form_has_correct_placeholders():
    ...
    assert form.fields['phone_number'].widget.attrs['placeholder'] == 'Phone Number *'
    assert form.fields['address_line_1'].widget.attrs['placeholder'] == 'Address Line 1 *'
    assert form.fields['address_line_2'].widget.attrs['placeholder'] == 'Address Line 2'
    assert form.fields['town_city'].widget.attrs['placeholder'] == 'Town or City *'
...
@pytest.mark.parametrize("field, value, expected_error", [
    # Invalid Email
    pytest.param("email", "johndoegmail.com", "Enter a valid email address.", id="missing_at_symbol"),
    pytest.param("email", "johndoe@gmailcom", "Enter a valid email address.", id="missing_dot_in_domain"),
    pytest.param("email", "johndoe@.com", "Enter a valid email address.", id="missing_local_part"),
    pytest.param("email", "@gmail.com", "Enter a valid email address.", id="missing_local_part_and_username"),
    pytest.param("email", "john.doe@com", "Enter a valid email address.", id="missing_subdomain"),
    pytest.param("email", "john..doe@gmail.com", "Enter a valid email address.", id="double_dot_in_local_part"),
    pytest.param("email", "john.doe@gmail..com", "Enter a valid email address.", id="double_dot_in_domain"),
    pytest.param("email", "john.doe@gmail.c", "Enter a valid email address.", id="domain_too_short"),
    pytest.param("email", "john.doe@.gmail.com", "Enter a valid email address.", id="dot_at_start_of_domain"),
    pytest.param("email", "john.doe.@gmail.com", "Enter a valid email address.", id="dot_at_end_of_local_part"),
    pytest.param("email", "john.doe@gmail .com", "Enter a valid email address.", id="space_in_domain"),
    pytest.param("email", "john.doe@ gmail.com", "Enter a valid email address.", id="space_before_domain"),

    # Invalid first name
    pytest.param("first_name", "", "This field is required.", id="first_name_required"),
    pytest.param("first_name", "Jayden Kasper Leroy Maximilian Ned", "Ensure this value has at most 30 characters (it has 34).", id="first_name_too_long"),

    # Invalid last name
    pytest.param("last_name", "", "This field is required.", id="last_name_required"),
    pytest.param("first_name", "Wolfeschlegelsteinhausenbergerdorff", "Ensure this value has at most 30 characters (it has 35).", id="first_name_too_long"),

    # Invalid phone number
    pytest.param("phone_number", "", "This field is required.", id="phone_number_required"),

    # Invalid address line 1
    pytest.param("address_line_1", "", "This field is required.", id="address_line_1_required"),

    # Invalid town city
    pytest.param("town_city", "", "This field is required.", id="town_city_required"),

    # Invalid country
    pytest.param("country", "", "This field is required.", id="country_required"),
])  
...
def test_update_order_status_form_invalid_data():
    ...
    assert form.errors['status'] == ['Select a valid choice. invalid is not one of the available choices.']