...
@pytest.mark.django_db
def test_cms_reviews_view_superuser(client, test_superuser, test_data_reviews):
    ...
    assert len(response.context["reviews"]) == 8


@pytest.mark.django_db
@pytest.mark.parametrize("status, expected_count, validity", [
    ("pending", 3, True),
    ("pending", 4, False),
    ("approved", 3, True),
    ("approved", 4, False),
...
def test_cms_reviews_view_filter_status(
  client, test_superuser, test_data_reviews, status, expected_count, validity
):
    user = test_superuser
    client.force_login(user)
...
@pytest.mark.django_db
def test_cms_review_update_view_customer(client, test_customer, test_data_reviews):
    user = test_customer.user
    client.force_login(user)
...
def test_cms_review_update_superuser(
  client, test_superuser, test_data_reviews
    ...
    assert response.status_code == 200
    assert "review/cms/review_update.html" in (t.name for t in response.templates)
...
def test_account_reviews_view_authenticated_with_reviews(
  client, test_customers, test_data_reviews, test_data_order
    ...
    assert "review/account/reviews.html" in [t.name for t in response.templates]
    assert 'reviews' in response.context

    user_reviews = Review.objects.filter(user=user)
    assert list(response.context['reviews']) == list(user_reviews)
