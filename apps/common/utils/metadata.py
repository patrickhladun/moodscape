...
class MetadataHandler:
    """
    Base class for metadata generation. Each specific metadata type (like Open Graph, Twitter)
    will inherit from this class and implement the `generate` method.
    """
...
class OpenGraphHandler(MetadataHandler):
    ...
    def generate(self, request, page, site_name, default_description):
        ...
        if "og" in page:
            ...
            og_tags.append(f'<meta property="og:site_name" content="{site_name}">')
            og_tags.append(f'<meta property="og:locale" content="{config.LOCALE}">')
            og_tags.append(f'<meta property="og:image" content="{og_image}">')
            og_tags.append(f'<meta property="og:url" content="{og_data.get("url", url)}">')
...
class TwitterHandler(MetadataHandler):
    ...
    def generate(self, request, page, site_name, default_description):
        ...
        if "twitter" in page:
            ...
            twitter_tags.append(f'<meta name="twitter:site" content="{config.TWITTER}">')
            ...
            twitter_tags.append(f'<meta name="twitter:image:alt" content="{twitter_data.get("image_alt", twitter_title)}">')
    ...
    def generate(self, request, page, site_name, default_description):
        ...
        if "product" in page:
            ...
            schema_data = {
                ...
                "@type": "Product",
                "name": product_data.get("name", page.get("title", site_name)),
            ...
            product_schema.append(f'<script type="application/ld+json">{schema_json}</script>')
...
class MetadataFactory:
    """
    The factory class that returns the appropriate metadata handler based on the type.
    """
    @staticmethod
...
def make_metadata(request, page):
    ...
    for metadata_type in ["standard", "og", "twitter", "product_schema"]:
        ...
        all_metadata.extend(handler.generate(request, page, site_name, default_description))
