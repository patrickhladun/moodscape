...
@pytest.mark.parametrize(
    "name,expected_slug",
    [
        pytest.param("Product 1", "product-1", id="generate_slug_simple"),
        pytest.param(
        ...
        pytest.param(
            "New Product!",
            "new-product",
            id="generate_slug_with_exclamation",
        ),
        pytest.param(
        ...
        pytest.param(
            "   Product 1   ",
            "product-1",
            id="generate_slug_with_extra_spaces",
        ),
        pytest.param("Product#1", "product1", id="generate_slug_with_hash"),
        pytest.param(
            "Amazing Product?",
            "amazing-product",
            id="generate_slug_with_question_mark",
        ),
        pytest.param(
            "Cool & Trendy Product",
            "cool-trendy-product",
            id="generate_slug_with_ampersand",
        ),
        pytest.param(
            "Test Product - Limited Edition",
            "test-product-limited-edition",
            id="generate_slug_with_dash",
        ),
        pytest.param(
            "Test Product + Special",
            "test-product-special",
            id="generate_slug_with_plus",
        ),
        pytest.param(
            "50% Off Product",
            "50-off-product",
            id="generate_slug_with_percent_sign",
        ),
        pytest.param(
            "Product (Special Edition)",
            "product-special-edition",
            id="generate_slug_with_parentheses",
        ),
        pytest.param(
            "Product_2024",
            "product_2024",
            id="generate_slug_with_underscore",
        ),
        pytest.param(
            "Product 2024-",
            "product-2024",
            id="generate_slug_without_ending_dash",
        ),
    ],
)
...
def test_product_form_details_field_type():
    form = ProductForm()
    assert (
        form.fields["details"].widget.__class__.__name__
        == "SummernoteWidget"
    )
