...
def test_constance_settings():
    ...
    assert (
        settings.CONSTANCE_CONFIG["COPYRIGHT"][0] == "All rights reserved."
    )
    assert (
        settings.CONSTANCE_CONFIG["SHOP_ADDRESS"][0]
        == "Moodscape Gallery, 456 Art Lane, Dublin, D04 EFG2, Ireland"
    )
    assert (
        settings.CONSTANCE_CONFIG["SHOP_MOBILE_PHONE"][0]
        == "+353 (0)85 425123"
    )
    assert (
        settings.CONSTANCE_CONFIG["SHOP_EMAIL"][0] == "support@moodscape.com"
    )
